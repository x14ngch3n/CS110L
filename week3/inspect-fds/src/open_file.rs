...
impl OpenFile {
    ...
    pub fn from_fd(pid: usize, fd: usize) -> Option<OpenFile> {
        let file_link = fs::read_link(format!("/proc/{}/fd/{}", pid, fd)).ok()?;
        let fdinfo = fs::read_to_string(format!("/proc/{}/fdinfo/{}", pid, fd)).ok()?;

        Some(OpenFile::new(
            OpenFile::path_to_name(file_link.to_str()?),
            OpenFile::parse_cursor(fdinfo.as_str())?,
            OpenFile::parse_access_mode(fdinfo.as_str())?,
        ))
    }
